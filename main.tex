...
A large number of innovative software tools (notebooks, virtual containers, active papers, dynamic figures, etc.) facilitate model publication, sharing, and re-use; and some researchers are writing articles and tutorials on how to use version control \cite<e.g.,>{blischak16}.
For example: the first article with an interactive figure has been recently published \cite{ogrean16}; the Jupyter notebook (\url{http://jupyter.org}) regarding the discovery of gravitational waves has been made public; and the binder project (\url{http://mybinderproject} THIS IS NOT A URL) offers online executable environments.
